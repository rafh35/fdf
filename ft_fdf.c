...
/*   Updated: 2019/02/23 10:22:44 by maberkan    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		fdf2_1(t_pos *t, t_var *v)
{
	...
	while (++a < t->nb)
	{
		v->y0 = ft_cavx(t->pos[a].x, t->pos[a].z) * t->ko / 2;
		v->y1 = ft_cavx(t->pos[a + 1].x, t->pos[a + 1].z) * t->ko / 2;
		v->x0 = ft_cavy(t->pos[a].y, t->pos[a].z) * t->ko / 2;
		v->x1 = ft_cavy(t->pos[a + 1].y, t->pos[a + 1].z) * t->ko / 2;
		if (t->pos[a].x == t->pos[a + 1].x)
			line(v, t, a);
	}
}
...
void		fdf2(t_pos *t, t_var *v)
{
	...
	{
		v->y0 = ft_cavx(t->pos[a].x, t->pos[a].z) * t->ko / 2;
		v->y1 = ft_cavx(t->pos[a + t->col].x, t->pos[a + t->col].z)
		* t->ko / 2;
		v->x0 = ft_cavy(t->pos[a].y, t->pos[a].z) * t->ko / 2;
		v->x1 = ft_cavy(t->pos[a + t->col].y, t->pos[a + t->col].z)
		* t->ko / 2;
		if (t->pos[a].x != t->pos[a + t->col].x && t->pos[a].y ==
		t->pos[a + t->col].y)
			line(v, t, a);
